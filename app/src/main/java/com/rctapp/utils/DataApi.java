...
import com.rctapp.models.VarietyVar;
...
public class DataApi {
    ...
    public static boolean UpdateProfileImage(String token, Context context, UserPreference userPreference) throws IOException, JSONException {
       String data = getUserInformation(token, context);
       JSONObject object = new JSONObject(data);
       JSONObject path = object.getJSONObject("data");
     //  JSONObject P = path.getJSONObject("profile_image_path");
        String img=path.getString("profile_image_path");
        if (!img.isEmpty()) {
            userPreference.setImage(img);
            Sucess = true;
        }
        Log.d("UpdateProfileImage: ", img);
        return  Sucess;
    }
 ...
 /*       UserPreference ref = new UserPreference(context);
    ...
    public static List<String> getVarietyLocation(Context context) {
        List<String> variety = new ArrayList<>();
        SharedPreferences sharedPreferences = context.getSharedPreferences("variety", Context.MODE_PRIVATE);
        String data = sharedPreferences.getString("variety", "null");
        try {
            ...
            for (int i = 0; i < array.length(); i++) {
                JSONObject o = array.getJSONObject(i);
                variety.add(o.getString("variety_name") + " from " + o.getString("platform_name") + " " + o.getString("platform_district"));
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public static boolean acceptTenderSeller(UserPreference userPreference, String tenderId){
        OkHttpClient client = new OkHttpClient.Builder()
                .connectTimeout (15, TimeUnit.MINUTES) // in seconds
                .readTimeout(15, TimeUnit.MINUTES)
                .sslSocketFactory(TrustAllSSL.createSSLSocketFactory())
        ...
        FormBody body = new FormBody.Builder()
                ...
                .build();
        Request request = new Request.Builder()
                .url(Api.main_url+Api.accept_tender+tenderId)
                .put(body)
                .addHeader("Authorization", "Bearer " + userPreference.getToken())
                .build();
        try {
            Response response = client.newCall(request).execute();
            Log.d( "acceptTender: ", response.body().string());
    ...
    public static boolean declineTender(UserPreference userPreference, String tenderId){
        OkHttpClient client = new OkHttpClient.Builder()
                .connectTimeout (15, TimeUnit.MINUTES) // in seconds
                .readTimeout(15, TimeUnit.MINUTES)
                .sslSocketFactory(TrustAllSSL.createSSLSocketFactory())
        ...
        FormBody body = new FormBody.Builder()
                ...
                .build();
        Request request = new Request.Builder()
                .url(Api.main_url+Api.decline_tender+tenderId)
                .put(body)
                .addHeader("Authorization", "Bearer " + userPreference.getToken())
                .build();
        try {
            Response response = client.newCall(request).execute();
            Log.d( "declineTender: ", response.body().string());
            if (response.code() == 200){
                Sucess = true;
            }
        } catch (IOException e) {
    ...
    public static boolean deleteTender(UserPreference userPreference, String tenderId){
        OkHttpClient client = new OkHttpClient.Builder()
                .connectTimeout (15, TimeUnit.MINUTES) // in seconds
                .readTimeout(15, TimeUnit.MINUTES)
                .sslSocketFactory(TrustAllSSL.createSSLSocketFactory())
    ...
    public static boolean deleteDp(UserPreference userPreference) throws IOException {
        OkHttpClient client = new OkHttpClient.Builder()
                .connectTimeout (15, TimeUnit.MINUTES) // in seconds
                .readTimeout(15, TimeUnit.MINUTES)
                .sslSocketFactory(TrustAllSSL.createSSLSocketFactory())
                ...
                .build();
        Request request = new Request.Builder()
                .url(Api.main_url+"/api/v1/service/file-manager/delete/"+userPreference.getUserId())
                .delete()
                .build();
        Response response = client.newCall(request).execute();
        Log.d("deleteDp: ", response.body().string());
        if (response.code() == 200){
            Sucess = true;
        }
